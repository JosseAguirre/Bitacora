...
public class ServicioAdmUsuario {
	...
	public List<AdmUsuario> findAll() {
		...
		Query q = em.createNativeQuery("SELECT id_usuario, usuario,contrasenia, id_empleado FROM bivi.adm_usuario");
		...
		for(Object[] row : rows){
			...
			us.setContrasenia(row[2].toString());
